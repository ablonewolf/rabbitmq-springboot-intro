...
public class FreeDeliverQueueConsumer {
    private static final Logger log = LoggerFactory.getLogger(FreeDeliverQueueConsumer.class);
    private final ObjectMapper objectMapper;

    @RabbitListener(queues = RabbitMQConfig.FREE_DELIVERY_QUEUE)
    public void listen(Message message) {
        try {
            var json = new String(message.getBody());
            var furniture = objectMapper.readValue(json, Furniture.class);
            log.info("Furniture received from the free delivery queue: {}", furniture);
